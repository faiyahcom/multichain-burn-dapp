...
export const useAmountActivity = (poolDetail?: PoolDetailResponse) => {
  ...
  const formattedBurned = useMemo(() => {
    ...
    return shortenNumber({
      number: Number(
        sciToFormatted(userAmount.deposited, pool.tokenInDecimals),
      ),
    });
  }, [pool, userAmount]);
  ...
  const formattedReward = useMemo(() => {
    ...
    return shortenNumber({
      number: Number(
        sciToFormatted(userAmount.claimed, pool.rewardTokenDecimals),
      ),
    });
  }, [pool, userAmount]);
  ...
  const formattedReturnReward = useMemo(() => {
    ...
    return shortenNumber({
      number: Number(
        sciToFormatted(pool.currentRewardAmount ?? 0, pool.rewardTokenDecimals),
      ),
    });
  }, [pool]);
