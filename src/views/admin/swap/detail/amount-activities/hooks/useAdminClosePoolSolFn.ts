...
export const useAdminClosePoolSolFn = () => {
    ...
    const adminClosePoolSol = useCallback(
        async ({ poolAddress, poolDetail }: AdminClosePoolSolParams) => {
            try {
                ...
                // ── 4. Owner reward ATA (goes back to pool owner) ──────────
                const poolOwner = new PublicKey(poolDetail.pool.owner);
