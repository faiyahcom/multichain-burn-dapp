...
const AdminRevenueFeeStats = () => {
    ...
    const fromParam = dateFrom ? String(Math.floor(dateFrom.getTime() / 1000)) : undefined;
    const toParam = dateTo ? String(Math.floor(dateTo.getTime() / 1000)) : undefined;
    const kindsParam =
        activeTab === "creation"
    ...
    const feeRows: FeeRow[] = useMemo(
        () =>
            (listData?.txns ?? []).map((record) => ({
                time: new Date(Number(record.timestamp) * 1000).toLocaleDateString(
                    "en-GB",
                ),
