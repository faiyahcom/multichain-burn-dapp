...
import type {
  TokenAllocation,
  WhitelistUser,
} from "@/services/whitelistUserService";
...
const formatTokenAmount = (amount: string, decimals: number): string => {
  try {
    ...
    return sigDigits
      ? `${whole.toLocaleString()}.${sigDigits}`
    ...
    <>
      ...
      >
        {visible.map((a, i) => (
          <span
            ...
            className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium whitespace-nowrap text-primary"
          >
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="overflow-hidden p-0 sm:max-w-150">
          ...
          {/* Network badge */}
          {networks.length > 0 && (
            <div className="flex items-center gap-1.5 px-6 pb-2 text-sm text-secondary-text">
          ...
          <div className="mx-4 mb-6 overflow-hidden border">
            <Table>
              <TableHeader>
                <TableRow className="rounded-sm bg-muted/50 hover:bg-muted/50">
                  <TableHead className="py-3 text-center font-semibold text-foreground">
                    Token
                  </TableHead>
                  <TableHead className="py-3 text-center font-semibold text-foreground">
                    Amount
              ...
              <TableBody>
                {allocations.map((a, i) => (
                  <TableRow
                    key={`${a.tokenAddress}-${i}`}
                    className="rounded-sm bg-muted/20 p-10 hover:bg-muted/30"
                  >
...
const UserNetworkIcons: React.FC<{ allocations: TokenAllocation[] }> = ({
  allocations,
}) => {
...
const AdminWhitelistUserTable: React.FC<Props> = ({ data }) => {
  ...
  const { disableWhitelistUser: disableSolana } =
    useDisableWhitelistUserSolanaFn();
        ...
        <TableBody>
          {isLoading && (
            <TableRow>
              <TableCell
                colSpan={7}
                className="py-10 text-center text-secondary-text"
              >
                Loading...
              </TableCell>
            </TableRow>
