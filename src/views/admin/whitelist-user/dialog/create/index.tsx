...
const buildSchema = (isSolana: boolean) =>
  z.object({
    ...
    walletAddress: z
      ...
      .refine((val) => (isSolana ? isSolanaAddress(val) : isEvmAddress(val)), {
        error: isSolana
...
const EVM_NETWORK_IDS = NETWORK_CONFIGS.filter(
  (n) => n.id !== SOLANA_NETWORK_ID,
).map((n) => n.id);
...
const AdminWhitelistUserDialogCreate = () => {
  ...
  const { createWhitelistUser: createSolana } =
  ...
  const onSubmit = async (data: WhitelistUserFormValues) => {
    ...
    if (
      !connectedNetworkCfg ||
      !selectedNetworks.includes(connectedNetworkCfg.id)
    ) {
    ...
    const remaining = selectedNetworks.filter(
      (n) => n !== connectedNetworkCfg.id,
    );
    if (remaining.length === 0) {
      ...
      toast.info(
        `Done for ${connectedNetworkCfg.label}. Switch network to continue.`,
      );
    }
  };
          ...
          <div className="space-y-2">
            ...
            {networkTypeConflict && (
              <p className="text-xs text-destructive">
                Cannot mix Solana and EVM networks — they use different address
                formats.
              </p>
            )}
          </div>
