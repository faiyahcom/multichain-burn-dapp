...
const ClosedStatus = ({ poolDetail, vaultBalance }: Props) => {
  ...
  const networkConfig = pool?.chainId
  ...
  const stakingRemainingRaw =
    poolDetail?.staking?.currentStaked ??
    poolDetail?.staking?.remainingStaked ??
  ...
  const formattedDepositAvailable =
    stakingRemainingRaw && pool?.tokenInDecimals !== undefined
      ? formatAmount(stakingRemainingRaw, pool.tokenInDecimals)
      : undefined;
