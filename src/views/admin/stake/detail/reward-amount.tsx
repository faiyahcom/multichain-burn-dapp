...
const StakedRewardAmount = ({ poolDetail }: Props) => {
  ...
  const claimedRewardRaw =
    poolDetail?.claimedRewardAmount ??
    staking?.totalClaimed ??
    staking?.user?.totalClaimed;
  const formattedClaimedReward = fmt(claimedRewardRaw, rewardDec);
  ...
  try {
    ...
    const claimedRaw = BigInt(claimedRewardRaw ?? "0");
