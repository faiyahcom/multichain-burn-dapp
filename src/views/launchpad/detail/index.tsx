...
import AmountActivity from "./amount-activity";
import { LAUNCHPAD_POOL_STATUS } from "@/types/admin/whitelist-token";
import { PoolStatusDisplay } from "@/components/shared/glow/pool/pool-status";
import type { LaunchpadPoolStatus } from "@/types/pool";
...
const LaunchpadPoolDetail = ({ address }: Props) => {
    ...
    const status = poolDetail?.pool?.status as LaunchpadPoolStatus | undefined;
    const safeStatus: LaunchpadPoolStatus = status ?? "upcoming";
        ...
        <div className="space-y-6 font-inter lg:space-y-17.5">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-6 lg:gap-10">
                {isLoadingPoolDetail ? (
                    ...
                    <>
                        ...
                        <PoolStatusDisplay variant="launchpad" className="w-3/7 min-w-20 px-2 py-1.5 text-sm sm:w-64 sm:text-base md:px-3 md:py-2 md:text-sm lg:px-5 lg:text-2xl 2xl:px-6 2xl:py-3">
                            {LAUNCHPAD_POOL_STATUS[safeStatus]?.label ?? safeStatus}
                        </PoolStatusDisplay>
            ...
            <div className="flex flex-col gap-4 lg:grid lg:grid-cols-3 lg:gap-x-6 lg:gap-y-9.5">
                ...
                <div className="order-2 lg:col-span-1 lg:row-span-3">
                    <AmountActivity poolDetail={poolDetail} />
                </div>
...
export default LaunchpadPoolDetail;
