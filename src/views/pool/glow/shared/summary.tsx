...
const PoolListGlowSummary = ({ poolKind }: Props) => {
    ...
    const cards: { title: string; value: string }[] = [
        ...
        },
        {
            ...
            value: Number(overallStats?.totalTransactions ?? 0).toLocaleString(
                "en-US",
            ),
        },
        {
            title: "Total Pools",
            value: Number(overallStats?.totalPools ?? 0).toLocaleString("en-US"),
        },
        {
            title: "Total Participants",
            value: Number(overallStats?.totalParticipants ?? 0).toLocaleString(
                "en-US",
            ),
        },
    ];
